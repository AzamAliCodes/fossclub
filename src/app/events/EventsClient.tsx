...
export default function EventsClient({ initialEvents = [] }: EventsClientProps) {
  // If initialEvents came from server, prime the memory cache
  if (initialEvents.length > 0 && !cachedEventsData) {
    cachedEventsData = initialEvents;
  }

  const [events, setEvents] = useState<ClubEvent[]>(() => {
    if (cachedEventsData && cachedEventsData.length > 0) return cachedEventsData;
    return initialEvents;
  });

  const [selectedEvent, setSelectedEvent] = useState<ClubEvent | null>(null);

  // Derive default tab directly from real initial data
  const [mainTab, setMainTab] = useState<"upcoming" | "past">(() => {
    const list = (cachedEventsData && cachedEventsData.length > 0) ? cachedEventsData : initialEvents;
    const hasUpcoming = list.some((e) => e.active);
    return hasUpcoming ? "upcoming" : "past";
  });

  // Loading skeleton is shown if we have no events yet
    ...
    fetch("/api/events")
      ...
      .then((data) => {
        if (data?.data && Array.isArray(data.data)) {
          ...
          setEvents(data.data);
          // Only automatically adjust default tab if initial render was empty
          if (events.length === 0) {
